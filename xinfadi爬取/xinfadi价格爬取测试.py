...
class price_spider(object):
    ...
    def parse_data(self,dic):
        price_list = dic['list']
        for test in range(0, len(price_list)):
            info = price_list[test]
            # print(info)
            self.prodName.append(info['prodName'])
    ...
    def save_data(self):
        rows = zip(self.prodName, self.lowPrice, self.avgPrice, self.highPrice, self.specInfo, self.place, self.unitInfo, self.pubDate)
